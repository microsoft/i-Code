...
class model_module(pl.LightningModule):
    ...
    def inference(self, xtype=[], condition=[], condition_types=[], n_samples=1, x_0=None, mix_weight={'video': 1, 'audio': 1, 'text': 1, 'image': 1}, image_size=256, ddim_steps=50, scale=7.5, num_frames=8):
        net = self.net
        ...
        for i, condition_type in enumerate(condition_types):
            if condition_type == 'image':
                ctemp1 = regularize_image(condition[i], augment=False).cuda()
                ...
                if x_0:
                    conditioning.append(torch.cat([uim, uim, cim]))
                else:    
                ...
                if scale != 1.0:
                    ...
                    uim = net.clip_encode_vision(dummy).cuda()
                if x_0:
                    conditioning.append(torch.cat([uim, cim, cim]))
                else:    
                    conditioning.append(torch.cat([uim, cim]))
                ...
                if x_0:
                    conditioning.append(torch.cat([uad, cad, cad]))
                else:    
                ...
                if x_0:
                    conditioning.append(torch.cat([utx, ctx, ctx]))
                else:    
        ...
        if x_0 is not None:  
            ...
            x_0 = regularize_image(x_0, 256, augment=False)[None]
            x_0 = x_0.to(conditioning[0])
            dummy = torch.zeros_like(x_0)
            u = self.net.autokl_encode(dummy)
            x_0 = self.net.autokl_encode(x_0)
            conditioning_x_0.append(torch.cat([u, u, x_0]))
        else:
            conditioning_x_0 = None
