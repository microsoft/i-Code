...
class DDIMSampler_VD(DDIMSampler):
    ...
    def p_sample_ddim(self, x, 
        ...
        num_cg_cases = len(condition[0])

        x_in = []
        for x_i in x:
            x_in.append(torch.cat([x_i] * num_cg_cases).clone().detach())

        x0_in = [None]
        if x_0 is not None and x_0_type == 'concat':
            for i in range(len(x_0)):
                x_in[i] = torch.cat([x_0[i].clone().detach(), x_in[i]], 2)
        elif x_0_type == 'cross_att':
            x0_in = x_0
            
        t_in = torch.cat([t] * num_cg_cases)
        
        out = self.model.model.diffusion_model(
            x_in, t_in, condition, xtype=xtype, condition_types=condition_types, x_0=x0_in, x_0_type=x_0_type, mix_weight=mix_weight)
        e_t = []
        for out_i in out:
            if num_cg_cases == 3:
                e_t_uncond1_uncond2_i, e_t_uncond1_i, e_t_i,  = out_i.chunk(3)
                e_t_i = e_t_uncond1_uncond2_i + unconditional_guidance_scale[0] * (e_t_uncond1_i - e_t_uncond1_uncond2_i)\
                        + unconditional_guidance_scale[1] * (e_t_i - e_t_uncond1_i)
            else:    
                e_t_uncond_i, e_t_i = out_i.chunk(2)
                e_t_i = e_t_uncond_i + unconditional_guidance_scale * (e_t_i - e_t_uncond_i)
            e_t_i = e_t_i.to(device)
            if x_0 is not None:
                e_t_i = e_t_i[:, :, -x_0[0].shape[2]:]
            e_t.append(e_t_i)
