...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x, return_full_seq=False, longer_idx=None):
        if (self.enable_fusion) and (
            self.fusion_type in ["daf_2d", "aff_2d", "iaff_2d"]
        ):
            global_x = x[:, 0:1, :, :]
...
class HTSAT_Swin_Transformer(nn.Module):
    ...
    def forward_features(self, x, return_full_seq=False, longer_idx=None):
        ...
        # print(2, return_full_seq, x.shape)
        if not return_full_seq:
            x = self.avgpool(x)
            x = torch.flatten(x, 1)
        # print(2, return_full_seq, x.shape)
        output_dict = {
    ...
    def forward(
        self, x: torch.Tensor, return_full_seq=False, mixup_lambda=None, infer_mode=False, device=None
        ...
        if not self.enable_fusion:
            ...
            output_dict = self.forward_features(x, return_full_seq=return_full_seq)
        else:
