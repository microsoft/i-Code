...
from core.common.utils import img_trans_torchvision, cache_from_blob, read_zip_file, get_visual_bbox
...
@dataclass
class MultilingualPdfDataTrainingArguments:
...
class MultilingualPdfDataset(Dataset):
    args: MultilingualPdfDataTrainingArguments
    mode: Split
    ...
    def __init__(
        ...
        self.use_cache = getattr(args, "cache_blob_json", False)
        self.use_zip = self.use_cache
        self.index_wrapper = list(range(len(self.examples)))
        self.whole_word_masking = task in ['text_and_layout_reconstruction', 'layout_modeling', 'visual_text_recognition']\
        ...
        if task == 'text_and_layout_reconstruction': 
        ...
        elif task == 'visual_text_recognition': 
    ...
    def get_filepath_with_cache(self, fpath):
        if self.use_cache:
            fp = cache_from_blob(fpath, use_zip=self.use_zip, is_main_process=self.is_world_process_zero())
            if fp:
                return fp
    ...
    def load_oneocr_example(self, fname, return_ocr=True, return_img=True):
        raw_ocr_path = os.path.join(self.data_dir, fname)
        ocr_path = self.get_filepath_with_cache(raw_ocr_path)

        ocr = None
        if return_ocr:
            if self.use_cache and self.use_zip and ocr_path.endswith(".zip"):
                try:
                    ocr = json.loads(read_zip_file(ocr_path))
                except:
                    ocr = json.load(open(raw_ocr_path, 'r', encoding='utf8'))
            else:
