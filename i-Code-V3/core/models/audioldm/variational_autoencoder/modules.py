...
# from core.models.audioldm.utils import instantiate_from_config
from core.models.audioldm.latent_diffusion.attention import LinearAttention
...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        h = hs[-1]
        h = self.mid.block_1(h, temb)
        if not self.swapped_to_float:
            self.mid.attn_1 = self.mid.attn_1.float()
            self.swapped_to_float = True
        h = self.mid.attn_1(h.float()).to(dtype)
        h = self.mid.block_2(h, temb)
        # end
        h = self.norm_out(h)
